...
export default function TestAgenticPage() {
  ...
  const fetchCredits = async () => {
    try {
      const res = await fetch(`${API_URL}/api/credits?user_id=default_user`);
      if (res.ok) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const connectEventSource = () => {
        ...
        eventSource = new EventSource(`${API_URL}/api/presentations/${data.job_id}/stream`);
      ...
      eventSource.addEventListener('slide_complete', (e) => {
        ...
        console.log('Full slide_complete data:', data);
        ...
        setSlides(prev => {
          ...
          console.log(`Updated slides array:`, newSlides.map((s, i) => `${i+1}: ${s?.substring(0, 50) || 'EMPTY'}...`));
          return newSlides;
        });
      ...
      eventSource.onerror = (error) => {
        ...
        if (retryCount < maxRetries) {
          retryCount++;
          addLog(`⚠️ Connection lost. Retrying... (${retryCount}/${maxRetries})`);
          ...
          addLog('❌ Connection lost after multiple retries');
          setIsGenerating(false);
          if (eventSource) eventSource.close();
        }
      };
  ...
  const handleOpenEditor = () => {
    ...
    console.log('Slides data:', slides.map((s, i) => `Slide ${i + 1}: ${s?.length || 0} chars`));
    
    if (slides.length === 0) {
      console.log('ERROR: No slides to edit');
      alert('No slides to edit. Please generate a presentation first.');
      return;
    }

    try {
      // Store slides in localStorage to pass to editor
      const slidesJson = JSON.stringify(slides);
      console.log('Storing in localStorage:', slidesJson.length, 'chars');
      localStorage.setItem('editorSlides', slidesJson);
      localStorage.setItem('editorJobId', jobId || '');
      
      // Verify storage
      const stored = localStorage.getItem('editorSlides');
      ...
      <div className="w-16 bg-gradient-to-b from-purple-700 to-blue-700 flex flex-col items-center py-6 space-y-6">
        ...
        <div className="flex-1 flex flex-col space-y-4">
          ...
          <NextLink href="/credits">
            <button className="w-10 h-10 rounded-lg bg-yellow-500/20 hover:bg-yellow-500/30 flex items-center justify-center transition-all group relative">
              <Coins className="w-5 h-5 text-yellow-300" />
              {credits !== null && (
                <div className="absolute -right-1 -top-1 bg-yellow-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
                  {credits > 99 ? '99+' : credits}
                </div>
              )}
            </button>
          </NextLink>
          <NextLink href="/database">
            <button className="w-10 h-10 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 flex items-center justify-center transition-all group relative" title="Database Viewer">
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white transition-all text-sm font-medium"
                  title="Edit Slides Manually"
                >
